 ...
 */
(function(c){var d=0;c.widget("ui.autocomplete",{options:{appendTo:"body",autoFocus:false,delay:300,minLength:1,position:{my:"left top",at:"left bottom",collision:"none"},source:null},pending:0,_create:function(){var b=this,f=this.element[0].ownerDocument,a;this.element.addClass("ui-autocomplete-input").attr("autocomplete","off").attr({role:"textbox","aria-autocomplete":"list","aria-haspopup":"true"}).bind("keydown.autocomplete",function(e){if(!(b.options.disabled||b.element.attr("readonly"))){a=false;var i=c.ui.keyCode;switch(e.keyCode){case i.PAGE_UP:b._move("previousPage",e);
break;case i.PAGE_DOWN:b._move("nextPage",e);break;case i.UP:b._move("previous",e);e.preventDefault();break;case i.DOWN:b._move("next",e);e.preventDefault();break;case i.ENTER:if(!b.menu.active){return}b.menu.select(e);break;case i.NUMPAD_ENTER:if(b.menu.active){a=true;e.preventDefault()}case i.TAB:if(!b.menu.active){return}b.menu.select(e);break;case i.ESCAPE:b.element.val(b.term);b.close(e);break;default:clearTimeout(b.searching);b.searching=setTimeout(function(){if(b.term!=b.element.val()){b.selectedItem=null;b.search(null,e)
}},b.options.delay);break}}}).bind("keypress.autocomplete",function(e){if(a){a=false;e.preventDefault()}}).bind("focus.autocomplete",function(){if(!b.options.disabled){b.selectedItem=null;b.previous=b.element.val()}}).bind("blur.autocomplete",function(e){if(!b.options.disabled){clearTimeout(b.searching);b.closing=setTimeout(function(){b.close(e);b._change(e)},150)}});this._initSource();this.response=function(){return b._response.apply(b,arguments)};this.menu=c("<ul></ul>").addClass("ui-autocomplete").appendTo(c(this.options.appendTo||"body",f)[0]).mousedown(function(e){var i=b.menu.element[0];
c(e.target).closest(".ui-menu-item").length||setTimeout(function(){c(document).one("mousedown",function(g){g.target!==b.element[0]&&g.target!==i&&!c.ui.contains(i,g.target)&&b.close()})},1);setTimeout(function(){clearTimeout(b.closing)},13)}).menu({focus:function(e,i){i=i.item.data("item.autocomplete");false!==b._trigger("focus",e,{item:i})&&/^key/.test(e.originalEvent.type)&&b.element.val(i.value)},selected:function(e,h){var i=h.item.data("item.autocomplete"),m=b.previous;if(b.element[0]!==f.activeElement){b.element.focus();
b.previous=m;setTimeout(function(){b.previous=m;b.selectedItem=i},1)}false!==b._trigger("select",e,{item:i})&&b.element.val(i.value);b.term=b.element.val();b.close(e);b.selectedItem=i},blur:function(){b.menu.element.is(":visible")&&b.element.val()!==b.term&&b.element.val(b.term)}}).zIndex(this.element.zIndex()+1).css({top:0,left:0}).hide().data("menu");c.fn.bgiframe&&this.menu.element.bgiframe()},destroy:function(){this.element.removeClass("ui-autocomplete-input").removeAttr("autocomplete").removeAttr("role").removeAttr("aria-autocomplete").removeAttr("aria-haspopup");
this.menu.element.remove();c.Widget.prototype.destroy.call(this)},_setOption:function(a,b){c.Widget.prototype._setOption.apply(this,arguments);a==="source"&&this._initSource();if(a==="appendTo"){this.menu.element.appendTo(c(b||"body",this.element[0].ownerDocument)[0])}a==="disabled"&&b&&this.xhr&&this.xhr.abort()},_initSource:function(){var b=this,f,a;if(c.isArray(this.options.source)){f=this.options.source;this.source=function(e,i){i(c.ui.autocomplete.filter(f,e.term))}}else{if(typeof this.options.source==="string"){a=this.options.source;
this.source=function(e,i){b.xhr&&b.xhr.abort();b.xhr=c.ajax({url:a,data:e,dataType:"json",autocompleteRequest:++d,success:function(g){this.autocompleteRequest===d&&i(g)},error:function(){this.autocompleteRequest===d&&i([])}})}}else{this.source=this.options.source}}},search:function(a,b){a=a!=null?a:this.element.val();this.term=this.element.val();if(a.length<this.options.minLength){return this.close(b)}clearTimeout(this.closing);if(this._trigger("search",b)!==false){return this._search(a)}},_search:function(a){this.pending++;
...
},_normalize:function(a){if(a.length&&a[0].label&&a[0].value){return a}return c.map(a,function(b){if(typeof b==="string"){return{label:b,value:b}}return c.extend({label:b.label||b.value,value:b.value||b.label},b)})},_suggest:function(a){var b=this.menu.element.empty().zIndex(this.element.zIndex()+1);this._renderMenu(b,a);this.menu.deactivate();this.menu.refresh();b.show();this._resizeMenu();b.position(c.extend({of:this.element},this.options.position));this.options.autoFocus&&this.menu.next(new c.Event("mouseover"))},_resizeMenu:function(){var a=this.menu.element;
a.outerWidth(Math.max(a.width("").outerWidth(),this.element.outerWidth()))},_renderMenu:function(b,f){var a=this;c.each(f,function(e,i){a._renderItem(b,i)})},_renderItem:function(a,b){return c("<li></li>").data("item.autocomplete",b).append(c("<a></a>").text(b.label)).appendTo(a)},_move:function(a,b){if(this.menu.element.is(":visible")){if(this.menu.first()&&/^previous/.test(a)||this.menu.last()&&/^next/.test(a)){this.element.val(this.term);this.menu.deactivate()}else{this.menu[a](b)}}else{this.search(null,b)}},widget:function(){return this.menu.element
...
a.select(d)}});this.refresh()},refresh:function(){var a=this;this.element.children("li:not(.ui-menu-item):has(a)").addClass("ui-menu-item").attr("role","menuitem").children("a").addClass("ui-corner-all").attr("tabindex",-1).mouseenter(function(d){a.activate(d,b(this).parent())}).mouseleave(function(){a.deactivate()})},activate:function(c,g){this.deactivate();if(this.hasScroll()){var k=g.offset().top-this.element.offset().top,e=this.element.attr("scrollTop"),a=this.element.height();if(k<0){this.element.attr("scrollTop",e+k)
}else{k>=a&&this.element.attr("scrollTop",e+k-a+g.height())}}this.active=g.eq(0).children("a").addClass("ui-state-hover").attr("id","ui-active-menuitem").end();this._trigger("focus",c,{item:g})},deactivate:function(){if(this.active){this.active.children("a").removeClass("ui-state-hover").removeAttr("id");this._trigger("blur");this.active=null}},next:function(a){this.move("next",".ui-menu-item:first",a)},previous:function(a){this.move("prev",".ui-menu-item:last",a)},first:function(){return this.active&&!this.active.prevAll(".ui-menu-item").length
},last:function(){return this.active&&!this.active.nextAll(".ui-menu-item").length},move:function(a,e,g){if(this.active){a=this.active[a+"All"](".ui-menu-item").eq(0);a.length?this.activate(g,a):this.activate(g,this.element.children(e))}else{this.activate(g,this.element.children(e))}},nextPage:function(a){if(this.hasScroll()){if(!this.active||this.last()){this.activate(a,this.element.children(".ui-menu-item:first"))}else{var g=this.active.offset().top,i=this.element.height(),e=this.element.children(".ui-menu-item").filter(function(){var c=b(this).offset().top-g-i+b(this).height();
return c<10&&c>-10});e.length||(e=this.element.children(".ui-menu-item:last"));this.activate(a,e)}}else{this.activate(a,this.element.children(".ui-menu-item").filter(!this.active||this.last()?":first":":last"))}},previousPage:function(a){if(this.hasScroll()){if(!this.active||this.first()){this.activate(a,this.element.children(".ui-menu-item:last"))}else{var e=this.active.offset().top,g=this.element.height();result=this.element.children(".ui-menu-item").filter(function(){var c=b(this).offset().top-e+g-b(this).height();return c<10&&c>-10
});result.length||(result=this.element.children(".ui-menu-item:first"));this.activate(a,result)}}else{this.activate(a,this.element.children(".ui-menu-item").filter(!this.active||this.first()?":last":":first"))}},hasScroll:function(){return this.element.height()<this.element.attr("scrollHeight")},select:function(a){this._trigger("selected",a,{item:this.active})}})})(jQuery);
/*
...
$.extend(Datepicker.prototype, {
	...
	_connectDatepicker: function(target, inst) {
		...
		//Set unselectable day if isTracking is true
		//Author Ruben Godoy
		if( inst.settings.isTracking) {
			this._disableDatesDatepicker( target );
		}
	},
	...
	_attachments: function(input, inst) {
		...
		var isTracking = this._get(inst, 'isTracking');
		
		if(isTracking){
			this._disableDatesDatepicker( inst );
		}
	},
	...
	_disableDatesDatepicker: function(target){
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						var idTdMonth=(printDate.getMonth())*1;idTdMonth=idTdMonth+1;
						
						tbody += '<td id="'+printDate.getDate()+'_'+idTdMonth+'_'+printDate.getFullYear()+'" class="' +
